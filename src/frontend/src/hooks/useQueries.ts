...
import type { UserProfile, UserFavorites, UserPreferences, ToolUsageRecord, SearchHistory } from '../backend';
...
export function useGetToolUsageRecords() {
  const { actor, isFetching } = useActor();

  return useQuery<ToolUsageRecord[]>({
    queryKey: ['toolUsageRecords'],
    queryFn: async () => {
      if (!actor) return [];
      return actor.getToolUsageRecords();
    },
    enabled: !!actor && !isFetching,
  });
}
...
export function useRecordToolUsage() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['toolUsageRecords'] });
...
export function useResetUserUsage() {
  ...
  return useMutation({
    mutationFn: async (userId: Principal) => {
      if (!actor) throw new Error('Actor not available');
      return actor.resetUserUsage(userId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['toolUsageRecords'] });
      queryClient.invalidateQueries({ queryKey: ['usageStats'] });
